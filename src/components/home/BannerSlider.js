...
const BannerSlider = ({ banners = PLACEHOLDER_BANNERS, onBannerPress }) => {
  ...
  const initialIndex =
    dataLength > 1 ? Math.floor(MULTIPLIER / 2) * dataLength : 0;
  const realIndexRef = useRef(initialIndex);
  const middleIndexRef = useRef(initialIndex);
  const totalItemsRef = useRef(extendedData.length);
  const currentDotRef = useRef(0);

  useEffect(() => {
    middleIndexRef.current = initialIndex;
    totalItemsRef.current = extendedData.length;
  }, [extendedData.length, initialIndex]);

  useEffect(() => {
    if (dataLength <= 1) {
      realIndexRef.current = 0;
      currentDotRef.current = 0;
      setCurrentIndex(0);
      return;
    }

    realIndexRef.current = initialIndex;
    currentDotRef.current = 0;
    setCurrentIndex(0);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const middleIndex = middleIndexRef.current;

      if (!totalItems || totalItems < 2) return;

      let baseIndex = realIndexRef.current;
      if (baseIndex < 0 || baseIndex >= totalItems) {
        baseIndex = middleIndex;
      }

      let nextIndex = baseIndex + 1;
      if (nextIndex >= totalItems) {
        // Seamlessly jump back to middle anchor without animation
  ...
  const onViewableItemsChanged = useRef(({ viewableItems }) => {
    if (viewableItems && viewableItems.length > 0) {
      ...
      if (index != null && dataLengthRef.current > 0) {
        ...
        const nextDotIndex = index % dataLengthRef.current;
        if (currentDotRef.current !== nextDotIndex) {
          currentDotRef.current = nextDotIndex;
          setCurrentIndex(nextDotIndex);
        }
      }
    }
  }).current;
  ...
  const renderItem = useCallback(
    ({ item }) => (
      ...
      >
        <Image
          source={{ uri: item.url || item.imageUrl || item.image?.url }}
  ...
  const keyExtractor = useCallback(
    (item, index) => String(item._uniqueId || item._id || item.id || index),
    [],
  );
  ...
  const handleScrollToIndexFailed = useCallback(
    ({ index }) => {
      const safeIndex = Math.min(
        Math.max(index, 0),
        Math.max(extendedData.length - 1, 0),
      );
