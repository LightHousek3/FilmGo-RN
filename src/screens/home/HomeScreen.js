...
const LocationButton = ({ value, onPress }) => (
    <TouchableOpacity onPress={onPress} activeOpacity={0.75} style={locStyles.btn}>
        ...
        {value && value !== 'Tất cả' ? <Text style={locStyles.btnText}>{value}</Text> : null}
    </TouchableOpacity>
);
...
const LocationItem = React.memo(({ name, isActive, onSelect }) => (
    ...
    >
        ...
        />
        <Text
            style={[locStyles.locationText, isActive && { color: COLORS.white, fontWeight: '700' }]}
        >
            {name}
        </Text>
        {isActive && (
...
const HomeScreen = ({ navigation }) => {
    ...
    const fetchData = useCallback(async (loc = null) => {
        try {
            ...
            const [nowShowingRes, comingSoonRes, promotionsRes, bannersRes, locationsRes, newsRes] =
